...
//                     auto code_point = border_code_point(node.direction, r - have_top_status_bar,
...
//             auto session_pointers = state.workspaces() | di::transform([](auto& p) {
//                                         return p.get();
//                                     });
...
//                 }); offset += session_name.size_bytes(); renderer.put_text(separator, status_bar_position, offset++,
//                 { .fg = label_bg, .bg = label_bg });
//             }
//
//             {
...
//             auto render_fn =
//                 Render(renderer, m_config.render, cursor, bg_color, tab, state, state.status_bar_position() ==
//                 0_u32);
//             render_fn(*tree);
//
