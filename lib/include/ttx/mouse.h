...
class MousePosition {
public:
    constexpr static auto from_pixels(MouseCoordinate pixels, Size const& size) -> MousePosition {
        if (size.rows == 0 || size.cols == 0) {
            return MousePosition({}, pixels);
        }

        // Determine the cell from pixel coordinates.
        auto in_cells = MouseCoordinate(pixels.x() * size.cols / size.xpixels, pixels.y() * size.rows / size.ypixels);
    ...
    constexpr auto in_pixels_with_fallback(Size const& size) const -> MouseCoordinate {
        ...
        auto x = (in_cells().x() * size.xpixels + size.xpixels / 2) / size.cols;
        auto y = (in_cells().y() * size.ypixels + size.ypixels / 2) / size.rows;
        return { x, y };
    }
    ...
    constexpr auto translate(MouseCoordinate offset_in_cells, Size const& size) const -> MousePosition {
        ...
        auto new_pixels = in_pixels().transform([&](MouseCoordinate pixels) {
            return MouseCoordinate {
                pixels.x() + size.xpixels / size.cols,
                pixels.y() + size.ypixels / size.rows,
            };
        });
